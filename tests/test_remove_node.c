...
static bool remove_func(void *a, void *b)
{
    return !strcmp(a, b);
}
...
Test(ll_remove_node_by_data, test_remove_node_by_data)
{
    node_t *node1 = ll_create_node("A");
    node_t *node2 = ll_create_node("B");
    node_t *node3 = ll_create_node("C");
    llist_t *list = ll_create_list();

    ll_push_front_node(list, node1);
    ll_insert_node(list, node2, 1);
    ll_append_node(list, node3);
    cr_assert_eq(ll_remove_node_by_data(list, "A", remove_func), true);
    cr_assert_eq(list->first, node2);
    cr_assert_eq(node2->prev, node3);
